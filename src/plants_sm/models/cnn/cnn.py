...
class CNN1D(nn.Module):
    def __init__(self, fc_hidden_sizes, cnn_layers, kernel_size, num_classes, last_sigmoid=True):
        ...
        for i in range(0, len(cnn_layers)):
            if i == 0:
                setattr(self, f"conv{i}", nn.Conv1d(in_channels=1, out_channels=cnn_layers[i], kernel_size=kernel_size[i]))
            else:
                setattr(self, f"conv{i}", nn.Conv1d(in_channels=cnn_layers[i-1], out_channels=cnn_layers[i], kernel_size=kernel_size[i]))
