...
from plants_sm.pathway_prediction.searcher import SearcherSolution
...
class TestMCTS_A(TestCase):
    ...
    def test_search(self):
        ...
        self.assertIsInstance(solution, SearcherSolution)

    def test_search_with_precursors(self):
        searcher = MCTS_A(reactors=[Retroformer()], device='cuda:1', simulations=100, cpuct=4.0, times=3000, predict_precursors=True)
        solution = searcher.search(molecule=Molecule.from_smiles("COC1=CC=C(C=C1)C2=COC3=CC(=C(C=C3C2=O)OC)O"))
        self.assertIsInstance(solution, SearcherSolution)